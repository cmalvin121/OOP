...
void Rockman::OnMove()
{
	...
	if ((isInjured) && (lastMovingState == 1) && (injureDelay == 30))//¸I¼²¨ì©Çª««á¡A¸}¦â©¹«á°h
	{
		x += STEP_SIZE * 5;
	}
	if ((isInjured) && (lastMovingState == 0) && (injureDelay == 30))//¸I¼²¨ì©Çª««á¡A¸}¦â©¹«á°h
	{
		x -= STEP_SIZE * 5;
	}
	...
	if (isMovingRight && !isKickWall && (!isInjured || injureDelay < 30)) //©¹¥k²¾°Ê
    {
        if ((keepkeydownSprinting) && (jumpDegree > 0))//¼u®g¸õÅD
            x += STEP_SIZE * 3;
...
void Rockman::isAlreadyOnGround()
{
    if (isDropping)
    {
        ...
        else if ((crashState_wall != 0) && (jumpDegree == 0)&&(!isInjured || injureDelay < 30))//½ðÀð·Æ¸¨
        {
            y += 10;
        }
...
void Rockman::OnShow()
{
	...
	else if (lastMovingState == 1 && isKickWall)//½ðÀð:¥ª
    {
    ...
    else if (lastMovingState == 0 && isKickWall)//½ðÀð:¥k
    {
